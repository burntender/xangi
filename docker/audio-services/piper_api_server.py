...
DEFAULT_MODEL_PATH = Path(
    os.getenv("PIPER_MODEL_PATH", "/app/models/tsukuyomi-chan-6lang-fp16.onnx")
)
DEFAULT_CONFIG_PATH = Path(os.getenv("PIPER_CONFIG_PATH", "/app/models/config.json"))
OPENAI_MODEL_ALIASES = {"tts-1", "tts-1-hd"}

class TTSRequest(BaseModel):
    model_config = ConfigDict(populate_by_name=True)

    model_id: str = Field(default=DEFAULT_MODEL_PATH.stem, alias="model")
    input: str
    speaker_id: int = 0
